...
def extract_icd_descriptions(db_cursor: cursor) -> pd.DataFrame:
    ...
    db_cursor.execute("SELECT * FROM icd_diagnoses")
...
def extract_icds(db_cursor: cursor) -> pd.DataFrame:
    """Extract ICD Codes"""
    db_cursor.execute('SELECT * FROM diagnoses_icd')
...
def extract_drgs(db_cursor: cursor) -> pd.DataFrame:
    """Extract DRG Codes"""
    db_cursor.execute("SELECT * from drgcodes")
...
def extract_admissions(db_cursor: cursor) -> pd.DataFrame:
    """Extract admissions"""
    db_cursor.execute('SELECT * FROM admissions')
...
def build_sql_query(table_module: str, table_name: str, id_type: str) -> str:
    ...
    #                    from ' + table_name + ' join (values {0}) \
    #                    as to_join(' + id_type + ') \
    #                    ON ' + table_name + '.' + id_type + ' \
    #                    = to_join.' + id_type

    sql_query = 'select ' +  table_name + '.* from ' + table_name + ' join (values {0}) as to_join(' + id_type + ') ON ' + table_name + '.' + id_type + ' \= to_join.' + id_type
    # sqlq=f"select {id_type} from {table_name}"
    sqlq=f"select * from {table_name}"
    return sqlq
...
def extract_admissions_for_admission_ids(db_cursor: cursor,
    ...
    sql_id_list = prepare_id_list_for_sql(hospital_admission_ids)
    sql_query = build_sql_query("mimic_hosp","admissions", "hadm_id")
    db_cursor.execute(sql_query.format(sql_id_list))

    adm = db_cursor.fetchall()
    cols = list(map(lambda x: x[0], db_cursor.description))
    x = pd.DataFrame(adm, columns=cols)
    print(cols,x.head)
    return pd.DataFrame(adm, columns=cols)
