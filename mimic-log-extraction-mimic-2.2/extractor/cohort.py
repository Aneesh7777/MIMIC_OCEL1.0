...
def extract_cohort(db_cursor: cursor, icd_codes: Optional[List[str]], icd_version: Optional[int],
    ...
    cohort["admittime"] = pd.to_datetime(cohort["admittime"])  # Convert "admittime" to datetime type
    cohort["admityear"] = cohort["admittime"].dt.year  # Now you can use .dt accessor
    ...
    # cohort["age"] = cohort["anchor_age"] + cohort["admityear"]  # type: ignore
    cohort["anchor_age"] = pd.to_numeric(cohort["anchor_age"])  # Convert "anchor_age" to integer
    cohort["age"] = cohort["anchor_age"] + cohort["admityear"]  # Now you can add the two columns


    # cohort["age"] = cohort["age"] - cohort["anchor_year"]  # type: ignore
    cohort["anchor_year"] = pd.to_numeric(cohort["anchor_year"])  # Convert "anchor_year" to integer
    cohort["age"] = cohort["age"] - cohort["anchor_year"]  # Now you can subtract the two columns
    ...
    if icd_codes is not None and icd_version is not None and icd_seq_num is not None:
        icd_cohort = filter_icd_df(icds=icds, icd_filter_list=icd_filter_list,icd_version=icd_version)
        # icd_cohort = icd_cohort.loc[(icd_cohort["seq_num"]) <= (icd_seq_num)]
        icd_cohort["seq_num"] = pd.to_numeric(icd_cohort["seq_num"], errors="coerce")
        icd_cohort = icd_cohort.loc[icd_cohort["seq_num"] <= icd_seq_num]
